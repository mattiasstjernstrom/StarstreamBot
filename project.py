...
def main():
    ...
    while True:
        find = input("What do you wan't to look up?: ").lower()
    ...
    if len(answer) == 3:
        ...
        print(f"I can't find anything related to: {find}")
        write_file(find)
...
def write_file(find):
    try:
        with open("requests.csv", "a", encoding="utf-8") as file:
            add_request = f"{find};1;{datetime.now()}\n"
            file.write(add_request)
            print(f"'{find}' is added to requests for learning purposes")
            return True
